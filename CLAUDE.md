...
13. **Tier 22 / 22.5 — unified status icons + preview drag-and-drop + cleanups**.
    ...
    **Per-board icon subsets — each board opts into ONLY the slots its
    ...
    |---|---|---|
    | **BB** (branch list) | HEAD, LOCAL, SYNCED, NOT-SYNCED | BB has per-branch VCS; all four states are native. HEAD fires on `branch.id === currentHeadId` (`= state.branchId`, the branch in the log editor) — NOT the list cursor `activeBranchId`. **LOCAL fires whenever `branch.isLocal` is true** (i.e. the branch has at least one record on this device) — independently of server presence. SYNCED fires when `isLocal && isServer && !isDirty`; ASYNCED fires when `isDirty \|\| (!isLocal && isServer)`. LOCAL + SYNCED therefore both render simultaneously for an in-sync local-and-server branch — they answer different questions ("does it live here?" vs. "is it in step with the server?"), so the icon legend mirrors the `vcs-list-owner` text 1:1 (`"local, online/{uid} [synced]"` → LOCAL + SYNCED, plus HEAD if active). Earlier code suppressed LOCAL on synced rows; that contradicted the text and was fixed. |
...
- `updatePage(subNaviItem)` — toggles `.active` on `.page` elements, controls push/pull buttons, head-indicator, feature scaffold based on CSS classes: `.can-push-pull`, `.show-branch`, `.have-feature`. Push/pull buttons hide via a transform-based **slide-behind animation** — `translateY(calc(-100% - 100vh))` (push behind header) / `translateY(calc(100% + 100vh))` (pull behind footer). The translation is intentionally a full-viewport-plus-button-height; an earlier button-height-relative `-256%` shrank with the `4vh` button on flat viewports and leaked the resting button into the page-container's clip box, overlapping list rows. **Do not** convert this to `visibility: hidden` — that kills the animation, which is the design. `inbox-thread.js`'s `_syncGlobalButtons` mirrors the same translateY values for member-count gating.
...
**DELETE PAGE handlers MUST cancel pending save + commit timers before mutating.** WT's `triggerCommit(text)` schedules `commitWE(text)` with a *captured* text snapshot from typing-time. If a delete fires after typing (200 ms save fired, 1 s commit still pending) and shifts `state.currentHead` onto the older record, the queued `commitWE(staleText) → WTVCS.save(state, staleText)` writes the typed-from-deleted-page text into the now-current older record — the user observes "delete deleted abc instead of def". Same pattern that `onAttach` already guards against. WT's `weDeleteBtn` handler now does `this.timers.cancel('save'); this.timers.cancel('commit')` before deleting. BB's input save reads `BBUI.getTextareaValue()` at fire-time (not capture-time), so it doesn't have this exact race; BC similarly reads live state. WT is unique because its commit timer captures the value at scheduling time.
