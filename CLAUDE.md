...
**File hash is name-sensitive: `SHA-256(content || 0x00 || original_name)`.** Two uploads with identical content but different filenames produce different hashes (no dedupe). Same content + same name → same hash → dedupe. This enables per-record file rename without a server rename endpoint: the chip name input is editable (Enter/blur commits), `editor-attachments._renameFile()` recomputes the hash under the new name, puts the blob under the new hash in `file_blobs` (status='local'), and fires `onRename(oldHash, newHash, meta)` so the host (BB/WT/BC) swaps the hash in the record's `file_hash` field. Next auto-commit uploads the new blob; the old server hash becomes orphan-eligible via the 24h cleanup. **Per-record uniqueness:** the same hash cannot appear twice in one record — attach/rename that would collide shows `files.duplicateInRecord` / `files.renameDuplicate` toast and is rejected. **Client and server must compute the same hash** — `FileService.upload(blob, filename)` sends the explicit filename so Blob uploads don't degrade to `"blob"` on the server. **BB attach / detach / rename all downgrade owner tag from `[synced]` to `[asynced]`** — each of these mutations makes the record reference content the server does not yet have (new file attached, existing file removed, or file renamed to a new hash). Leaving the tag as `[synced]` would mislead observers (including other tabs) into seeing a branch that looks server-consistent while local content has diverged. All three paths use the shared `markAsynced` helper in `blackboard-core.js`; the composite primary key `[owner+branch_id+timestamp]` forces `.modify()` to delete+put, so state.owner is re-aligned if it matched the old owner. Commit re-promotes back to `[synced]` once upload succeeds. **Caveat on the visible branch-list tag:** `blackboard-ui.js` renders `[synced]`/`[asynced]` from `branch.isDirty`, which compares local max record timestamp against server max record timestamp — so the owner-column flip alone does not visibly repaint the tag until a timestamp-changing event (text edit, commit) follows. The flip still matters for cross-tab correctness (other tabs' `state.owner` re-derivation) and for the eventual commit tag promotion.

**File chip icon vs download button.** Icon is `<a href="/api/files/{hash}?inline=1" target="_blank" rel="noopener">` — browser opens the file inline in a new tab (PDF/image/text preview). Native anchor navigation preserves the user gesture, which `window.open()` after awaits would lose to popup blockers. Download button (`[⬇]`) triggers a blob Save-As via `downloadFile()` + `_saveBlobAs()`. LOCAL-only chips (server has no blob yet) intercept the anchor click, preventDefault, and pre-open a blank tab synchronously before awaiting the blob to preserve the gesture.
...
### Test Suite (277 tests, 633 assertions)
