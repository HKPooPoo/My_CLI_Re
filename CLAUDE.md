...
- `updatePage(subNaviItem)` — toggles `.active` on `.page` elements, controls push/pull buttons, head-indicator, feature scaffold based on CSS classes: `.can-push-pull`, `.show-branch`, `.have-feature`. Push/pull buttons are hidden via THREE properties together: `transform: translateY(±256%)` (animation), `visibility: hidden`, and `pointer-events: none`. The translateY alone is `vh`-based and a sliver can leak inside `.page-container`'s clip box at flat (short-height) viewports; `visibility:hidden` is the belt-and-braces guarantee they can't paint over list rows underneath. `inbox-thread.js`'s `_syncGlobalButtons` mirrors the same trio for member-count gating.
